...
public class Test {
		...
		try {
			...
			while (str != null) {
				...
				if (str != null) {
					...
					LocalDateTime inputDate = LocalDateTime.parse(data[0], DateTimeFormatter.ofPattern("yyyy/MM/dd HH:mm:ss")); // 変換した形式.
					...
					} else if (500 < responseSpeed && responseSpeed <= 2000) {
	 ...
	 * @param inputCount 回数
	 */
	private void resultToConsole(LocalTime logStartTime, int countOfResponseSpeedUnder500, int countOfResponseSpeedUnder2000,
			int countOfResopnseSpeedOver2001, int sum, int inputCount) {
